...
3. **Never** send, pay, submit, publish or run state-changing git. Those are human acts. RED stops and emits the three-line handoff card from the session card. Cowork mode (`sync --without git`) lifts the git half of this line and nothing else.
...
| Path | Holds | Changed |
|---|---|---|
| `plugins/handoff-os/` | 3 skills, 1 scout agent, 6 hooks, 5 scripts — the product | on owner order |
