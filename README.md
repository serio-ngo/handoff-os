...
<!-- handoff-demo -->
<img src="docs/demo.svg" width="1168" alt="handoff-os session: Read src/big.js · 35 KB → HANDOFF OS: big.js is 35KB; trimmed to its first 384 lines · Workflow · 100 agents → FAN-OUT CAP: 100 subagents requested, wave capped at 3 — override: HANDOFF_MAX_PER_WAVE=100 (restart) · Agent model:opus · &quot;review the diff&quot; → DISPATCH BUDGET: blocked an opus review · gmail send_message → EGRESS LOCK: blocked mcp__gmail__send_message — &quot;send&quot; leaves the org or destroys a record — override: HANDOFF_MCP_ALLOW=send_message (restart) · Agent model:sonnet · &quot;review src/parse.js&quot;, allowed · HANDOFF OS · 1 trimmed · 100 held · 1 redirected · 1 blocked · 1 dispatched · ~2,816 of ~8,960 tok kept out (31%) · all-time ~146.2k tok">
<!-- /handoff-demo -->
...
2. `npm run setup`
   You should see steps `[1/4]` to `[4/4]` end with `Ready. Restart Claude Code so the session card loads.`
...
| Guide mode | `HANDOFF_GUIDE=0` disables — guide mode: next-step and override hints on the human channel. On by default. |

Every guard blocks first and suggests second. No block is a refusal of the work: the session names
the way through — the override above, or the command to run yourself — and the owner decides. What
the owner then spends, on any tier or any number of subagents, is the owner's call.
