import {
  CONNECTOR_ALLOW, DESTRUCTIVE, OUTWARD, OUTWARD_PREFIX, READ_PREFIX, RESTORATIVE, STRONG, WRITE_VERBS,
} from './patterns.mjs';
...
export function judgeConnector(tool, input) {
  ...
  const hit = [...DESTRUCTIVE, ...WRITE_VERBS, ...OUTWARD].find((verb) => action.includes(verb))
    || (OUTWARD_PREFIX.test(dashed) ? 'request' : undefined);
  const allowed = RESTORATIVE.test(dashed)
    || (READ_PREFIX.test(dashed) && !STRONG.some((verb) => action.includes(verb)))
    || CONNECTOR_ALLOW.some((rx) => rx.test(dashed));
  return hit && !allowed ? `blocked ${tool} — "${hit}" sends outward or destroys a record` : null;
}
